    ...
    : mTolerance(Tolerance)
{
    ...
    else if (Comparer.IsNotEqual())
    {
        ...
        {
            mSpans.emplace_back(-CNumber::Infinity, Right, ESpanType::closed, ESpanType::open);
            mSpans.emplace_back(Right, CNumber::Infinity, ESpanType::open, ESpanType::closed);
        }
    }
...
void CRange::Intersect(EComparer Comparer, CNumber Right)
{
    if (Comparer.IsNotEqual())
    {
        ...
        {
            CRange LeftSpan = GetIntersection(SSpan(-CNumber::Infinity, Right, ESpanType::closed, ESpanType::open));
            CRange RightSpan = GetIntersection(SSpan(Right, CNumber::Infinity, ESpanType::open, ESpanType::closed));
            mSpans.clear();
    ...
    else if (Comparer.IsNotAlmostEqual())
    {
        if (Right.IsNaN())
        {
            // Do nothing because x≄NaN ≡ -∞≤x≤+∞ and the intersection of any number and extended real numbers is still itself.
        }
        else if (Right.IsPositiveInfinity())
        {
            Intersect(SSpan::PositiveInfinityExclusion);
...
void CRange::Union(EComparer Comparer, CNumber Right)
{
    if (Comparer.IsNotEqual())
    {
        ...
        {
            Union(SSpan(-CNumber::Infinity, Right, ESpanType::closed, ESpanType::open));
            Union(SSpan(Right, CNumber::Infinity, ESpanType::open, ESpanType::closed));
        }
    }
    else if (Comparer.IsNotAlmostEqual())
    {

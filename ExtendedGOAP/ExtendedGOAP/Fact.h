...
namespace ExtendedGOAP
{
    ...
    {        
        ...
        friend class CMatrixFact;
        friend class CNumericFact;
        friend class CSetFact;
    public:
        virtual ~CFact() = 0 {}
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
    class CMatrixFact : public CFact // Matrix fact used to define preconditions and effects with C++ operators
    {
        friend class CFactDefinition;
    public:
        CFactAssignment operator = (const CMatrixFact& Right) const;
        CFactAssignment operator = (const CMatrixExpression& Right) const;

        template <RMatrixArgument T>
        CFactAssignment operator += (T&& Right) const { return {*this, EOperator::matrixAddition, std::forward<T>(Right)}; }
        template <RMatrixArgument T>
        CFactAssignment operator -= (T&& Right) const { return {*this, EOperator::matrixSubtraction, std::forward<T>(Right)}; }
        template <RMatrixArgument T>
        CFactAssignment operator *= (T&& Right) const { return {*this, EOperator::matrixMultiplication, std::forward<T>(Right)}; }
        template <RNumericArgument T>
        CFactAssignment operator *= (T&& Right) const { return {*this, EOperator::scalarMultiplication, std::forward<T>(Right)}; }

        CNumber GetGapWeight() const override { return mGapWeight; }
        void SetGapWeight(CNumber GapWeight);

    private:
        CMatrixFact(CFactDefinition& Owner, int Index, const std::string& Name, CNumber GapWeight);

    private:
        CNumber mGapWeight = 1;
    };
    ///////////////////////////////////////////////////////////////////////////////////////////////
    class CSetFact : public CFact // Set fact used to define preconditions and effects with C++ operators
    {
        friend class CFactDefinition;
    public:
        CFactAssignment operator = (const CSetFact& Right) const;
        CFactAssignment operator = (const CSetExpression& Right) const;

        template <RSetArgument T>
        CFactAssignment operator &= (T&& Right) const { return {*this, EOperator::setIntersection, std::forward<T>(Right)}; }
        template <RSetArgument T>
        CFactAssignment operator |= (T&& Right) const { return {*this, EOperator::setUnion, std::forward<T>(Right)}; }
        template <RSetArgument T>
        CFactAssignment operator -= (T&& Right) const { return {*this, EOperator::setDifference, std::forward<T>(Right)}; }
        template <RSetArgument T>
        CFactAssignment operator ^= (T&& Right) const { return {*this, EOperator::setSymmetricDifference, std::forward<T>(Right)}; }

        CNumber GetGapWeight() const override { return mGapWeight; }
        void SetGapWeight(CNumber GapWeight);

    private:
        CSetFact(CFactDefinition& Owner, int Index, const std::string& Name, CNumber GapWeight);
    ...
    public:
        ...
        CMatrixFact* DefineMatrix(const std::string& Name, CNumber GapWeight = 1);
        CSetFact* DefineSet(const std::string& Name, CNumber GapWeight = 1);

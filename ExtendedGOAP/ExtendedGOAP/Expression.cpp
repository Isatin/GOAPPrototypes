...
///////////////////////////////////////////////////////////////////////////////////////////////
CFactInfo::CFactInfo(const CFact& Fact)
    : mIndex(static_cast<int16_t>(Fact.GetIndex()))
    , mType(Fact.GetType())
{}
///////////////////////////////////////////////////////////////////////////////////////////////////
struct CExpression::SRepeatedOperation
{
    ...
    std::vector<CFactInfo> FactInfos;           // Information of referenced facts 
...
{
    return CExpression(Operator, FactInfos, OperationIndexes, Constatnt, SourceOperationIndex);
}
...
struct CExpression::SSummation
{
    ...
    std::vector<CFactInfo> AddendFactInfos;         // Fact information of the addends 
    std::vector<int> AddendOperationIndexes;        // Operations indexes of the addends
    std::vector<CFactInfo> SubtrahendFactInfos;     // Fact information of the subtrahends 
...
CExpression CExpression::SSummation::ToExpression() const
{
    ...
    for (int i = 0; i < SubtrahendFactInfos.size(); i++)
    {
        Opposite.mOperations.emplace_back(EOperator::minus, SubtrahendFactInfos[i]);
    }
    ...
    CExpression Addition(EOperator::addition, AddendFactInfos, AddendOperationIndexes, Sum, AdditionStartOpIndex);
...
struct CExpression::SFraction
{
    ...
    std::vector<CFactInfo> NumeratorFactInfos;      // Fact information of the numerator 
    std::vector<int> NumeratorOperationIndexes;     // Operations indexes of the numerator
    std::vector<CFactInfo> DenominatorFactInfos;    // Fact information of the denominator 
...
CExpression CExpression::SFraction::ToExpression() const
{
    ...
    CExpression Denominator(EOperator::multiplication, DenominatorFactInfos, DenominatorOperationIndexes, CNumber::Null, SourceOperationIndex);
    ...
    CExpression Numerator(EOperator::multiplication, NumeratorFactInfos, NumeratorOperationIndexes, Coefficient, SourceOperationIndex + DenominatorOpCount);
...
COperand::COperand(CFactInfo FactInfo)
    : mType(EOperand::fact)
    , mFactInfo(FactInfo)
{}
...
bool COperand::operator == (const COperand& Another) const
{
    ...
    case EOperand::fact:        return mFactInfo == Another.mFactInfo;
    case EOperand::operation:   return mOperationIndex == Another.mOperationIndex;
...
const CFactInfo& COperand::GrabFactInfo() const
{
    assert(IsFact());
...
EFactType COperand::GrabFactType() const 
{ 
    assert(IsFact());

    return mFactInfo.GetType(); 
}
...
CExpression::CExpression(EOperator Operator, const std::vector<CFactInfo>& FactInfos, const std::vector<int>& OperationIndexes, CNumber Constant, int StartOperationIndex)
{
    assert(Operator.GetArity() > 0);
    assert(Operator.GetArity() != 1 || FactInfos.size() + OperationIndexes.size() + (Constant.IsNull() ? 0 : 1) <= 1);
    ...
    if (FactInfos.size() >= 2)
    {
        mOperations.emplace_back(Operator, FactInfos[IndexOfFactIndexes], FactInfos[IndexOfFactIndexes + 1]);
        IndexOfFactIndexes += 2;
    }
    else if (FactInfos.size() == 1)
    {
        if (OperationIndexes.size() >= 1)
        {
            mOperations.emplace_back(Operator,
                FactInfos[IndexOfFactIndexes++],
                SOperationIndex(OperationIndexes[IndexOfOpIndexes++]));
        }
        else if (!Value.IsNull())
        {
            mOperations.emplace_back(Operator, FactInfos[IndexOfFactIndexes++], Value);
            Value = CNumber::Null;
        }
        else
        {
            mOperations.emplace_back(EOperator::nil, COperand::Null, FactInfos[IndexOfFactIndexes++]);
        }
    }
    ...
    for (; IndexOfFactIndexes < FactInfos.size(); IndexOfFactIndexes++)
    {
        mOperations.emplace_back(Operator,
...
bool CExpression::ReturnScalar() const
{
    if (mOperations.empty())
    {
        return false;
    ...
    auto IsScalar = [&](this auto& Self, const SOperation& Op)
    {
        if (Op.HasOperator())
        {
            return Op.Operator.IsScalar().IsYes();
        }
        else if (Op.Right.IsFact())
        {
            return Op.Right.GrabFactType().IsScalar();
        }
        else if (Op.Right.IsConstant())
        {
            return true;
        }
        else if (Op.Right.IsOperation())
        {
            return Self(mOperations[Op.Right.GrabOperationIndex()]);
        }
        else if (Op.Right.IsNull())
        {
            assert(!"Right operand must NOT be null.");
            return false;
        }
    ...
    return IsScalar(mOperations.back());
}

bool CExpression::ReturnMatrix() const
{
    if (mOperations.empty())
    {
        return false;
    ...
    auto IsMatrix = [&](this auto& Self, const SOperation& Op)
    {
        if (Op.HasOperator())
        {
            return Op.Operator.ReturnMatrix();
        }
        else if (Op.Right.IsFact())
        {
            return Op.Right.GrabFactType() == EFactType::matrix;
        }
        else if (Op.Right.IsConstant())
        {
            return false;
    ...
    return IsMatrix(mOperations.back());
}

bool CExpression::ReturnSet() const
{
    if (mOperations.empty())
    {
        return false;
    ...
    const SOperation& Op = mOperations.back();
    auto IsSet = [&](this auto& Self, const SOperation& Op)
    {
        if (Op.HasOperator())
        {
            return Op.Operator.ReturnSet();
        }
        else if (Op.Right.IsFact())
        {
            return Op.Right.GrabFactType() == EFactType::set;
        }
        else if (Op.Right.IsConstant())
        {
            return false;
...
CBitVector CExpression::GetUsedFactBits() const
{
    ...
    for (const SOperation& Op : mOperations)
    {
        if (Op.Left.IsFact())
        {
            Return.GetOrAdd(Op.Left.GrabFactIndex()) = true;
        }
        if (Op.Right.IsFact())
        {
            Return.GetOrAdd(Op.Right.GrabFactIndex()) = true;
        }
    }
...
CVariant CExpression::Evaluate(const CStateBase& State) const
{
    if (mOperations.empty())
    {
    ...
    std::vector<CVariant> Results(mOperations.size());
    auto FetchValue = [&](CVariant& oOutput, const COperand& Operand)
    {
        switch (Operand.GetType())
        {
        case EOperand::null:        oOutput = CVariant::Unset; break;
        case EOperand::fact:        oOutput = State.GetProperty(Operand.GrabFactIndex()); break;
        case EOperand::operation:   oOutput = Results[Operand.GrabOperationIndex()]; break;
        case EOperand::constant:    oOutput = Operand.GrabConstant(); break;
        default:                    assert(!"Invalid operand type"); break;
        }        
    };
    ...
    for (int i = 0; i < mOperations.size(); i++)
    {
        ...
        CVariant RightValue;
        FetchValue(RightValue, Op.Right);
        if (RightValue.IsUnset())
        {
            return RightValue;
        }

        Results[i] = Op.Operator(std::move(LeftValue), std::move(RightValue));
    }
...
void CExpression::ReplaceFact(int FactIndex, const CExpression& Replacement)
{
    ...
    auto ShiftOrReplace = [=](COperand& Operand)
    {
        ...
        {
        case EOperand::fact:
            if (Operand.GrabFactIndex() == FactIndex)
            {
        ...
        case EOperand::operation:
            Operand.mOperationIndex += ReplacementSize;
            break; 
        }
    };
...
int CExpression::RemoveOperations(int StartOperationIndex, int EndOperationIndex)
{
    ...
    for (int OpIndex = StartOperationIndex; OpIndex < mOperations.size(); OpIndex++)
    {
        ...
        if (Op.Left.IsOperation())
        {
            Op.Left.mFactInfo.mIndex -= Delta;
        }
        if (Op.Right.IsOperation())
        {
            Op.Right.mOperationIndex -= Delta;
        }
    }
...
void CExpression::RearrangeSetOperations()
{
    // TODO: Apply set theory laws to transform equivalent set expressions into the same form, 
    // such as the double complement, De Morgan's, domination, idempotent, complement, and absorption laws.
}
...
void CExpression::RearrangeFractions()
{
    ...
    auto CollectFraction = [](SFraction& Fraction, const COperand& Operand, bool Reciprocal)
    {
        if (Operand.IsFact())
        {
            if (Reciprocal)
            {
                Fraction.DenominatorFactInfos.push_back(Operand.GrabFactInfo());
            }
            else
            {
                Fraction.NumeratorFactInfos.push_back(Operand.GrabFactInfo());
            }
        }
        else if (Operand.IsOperation())
        {
...
void CExpression::RearrangeSubtractions()
{
    ...
    auto CollectSummation = [](SSummation& Summation, const COperand& Operand, bool Negative)
    {
        if (Operand.IsFact())
        {
            if (Negative)
            {
                Summation.SubtrahendFactInfos.push_back(Operand.GrabFactInfo());
            }
            else
            {
                Summation.AddendFactInfos.push_back(Operand.GrabFactInfo());
            }
        }
        else if (Operand.IsOperation())
        {
...
void CExpression::RearrangeCommutativeOperations()
{
    ...
    auto CollectRepeatedOperation = [](SRepeatedOperation& RepeatedOperation, const COperand& Operand)
    {
        if (Operand.IsFact())
        {
            RepeatedOperation.FactInfos.push_back(Operand.GrabFactInfo());
        }
        else if (Operand.IsOperation())
        {
    ...
    for (int RepeatedOpIndex = static_cast<int>(RepeatedOps.size()) - 1; RepeatedOpIndex >= 0; RepeatedOpIndex--)
    {
        ...
        std::vector<CFactInfo>& FactInfos = RepeatedOp.FactInfos;
        ...
        std::ranges::sort(RepeatedOp.FactInfos, CFactInfo::Compare);

        if (OpIndexes.size() == 2 && FactInfos.empty()
...
CNumber CExpression::SplitInto(std::vector<STerm>& oTerms, CNumber OuterScale) const
{
    ...
    for (int i = 0; i < ScaledIndexes.size(); i++)
    {
        ...
        {
            if (Op.IsAddition() || Op.IsMatrixAddition())
            {
                AddTerm(Op.Left, Scale);
                AddTerm(Op.Right, Scale);
            }
            else if (Op.IsSubtraction() || Op.IsMatrixSubtraction())
            {
                AddTerm(Op.Left, Scale);
                AddTerm(Op.Right, -Scale);
            }
            ...
            else if (Op.Left.IsConstant() && (Op.IsMultiplication() || Op.IsScalarMultiplication()))
            {
                ...
                (Op.IsMultiplication() || Op.IsScalarMultiplication() || (Op.IsDivision() && Op.Right.GrabConstant() != 0)))
            {

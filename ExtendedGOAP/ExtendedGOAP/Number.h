...
namespace ExtendedGOAP
{
    ...
    public:
        ...
        static constexpr size_t GetOrderedNumberCount(); // The number of unequal floating-point values excluding NaN

        static CNumber GetDefaultTolerance() { return DefaultTolerance; }
        static void SetDefaultTolerance(CNumber Value);

        static bool IsSame(CNumber Left, CNumber Right) { return Left.mValue == Right.mValue; }
    ...
    constexpr size_t CNumber::GetOrderedNumberCount()
    {
        static_assert(sizeof(size_t) >= sizeof(BValue));

        size_t NaNCount = (1LL << std::numeric_limits<BValue>::digits) - 2; // Exclude the two infinities. 
        typename SSizeToUInt<sizeof(BValue)>::Type MaxInt = ~0;             // The maximum unsigned integer equals 2^n - 1.
        size_t BitStringCountMinusOne = MaxInt;                             // Exclude the positive or negative zero.
        return BitStringCountMinusOne - NaNCount;
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////////
namespace std
{
    ...
    template <>
    struct hash<ExtendedGOAP::CNumber>
    {
        size_t operator () (ExtendedGOAP::CNumber Source) const noexcept
        {
            return hash<ExtendedGOAP::CNumber::BValue>{}(Source.Get());
        }
    };
    ///////////////////////////////////////////////////////////////////////////////////////////////
}

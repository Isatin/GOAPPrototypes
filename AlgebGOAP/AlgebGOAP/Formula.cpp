    ...
    for (int CompoundIndex = static_cast<int>(mCompounds.size()) - 1; CompoundIndex >= 0; CompoundIndex--)
    {
        ...
        else if (Left.IsAtomic())
        {
            if (Right.IsAtomic())
            {
                ...
                assert(LeftRelation.GetRight().IsOnlyOneConstant());
                assert(RightRelation.GetRight().IsOnlyOneConstant());
...
void CFormula::ReduceUnconditionalAndUnsatisfiableInequalities()
{
    ...
    // NOTE: Call RemoveRedundantBoolsAndAtomicCompounds later to remove the redundant Boolean constants generated by this function.
